// scripts/screenshot-settings-documents.mjs
// Zrzuty ekranu sekcji „Dokumenty i podpisy" w Ustawieniach, z DZIAŁAJĄCEJ
// aplikacji.
//
...
//
// Uruchomienie:  node scripts/screenshot-settings-documents.mjs [baseUrl]
import { chromium } from 'playwright';
import { mkdirSync } from 'node:fs';

const BASE = process.argv[2] ?? 'http://localhost:5173';
const OUT = process.env.SHOT_DIR ?? 'screenshots/settings-documents';
...
export async function stub(context) {
    ...
    await context.route((url) => url.pathname.startsWith('/api/'), async (route) => {
        ...
        if (path === '/v1/company/document-logo-config') {
            return json(route, { showLogoOnDocuments: true, hasLogo: true });
        }
        // Reguły dokumentów, szablony i zgody - listy; pusty obiekt z domyślnej
        // gałęzi wywracałby sekcję na `.map`.
        if (path.startsWith('/v1/protocol-rules') || path.startsWith('/v1/protocol-templates')
...
if (isMain) await (async () => {
    ...
    for (const shot of SHOTS) {
        ...
        await page.goto(`${BASE}/settings?tab=documents`, { waitUntil: 'networkidle' });
        await page.waitForSelector(process.env.SHOT_WAIT ?? 'text=umieszczać logo', { timeout: 20_000 });
        await page.waitForTimeout(800);
        // Karta logo siedzi na końcu sekcji - bez tego zrzut łapie jej górę
        // gdzieś poza kadrem.
        await page.getByText('umieszczać logo').first().scrollIntoViewIfNeeded();
