    ...
    items: [
      {
        ...
        group: 'Wstęp',
        description: 'Lead to potencjalny klient — osoba, która wysłała zapytanie przez telefon, e-mail lub ręcznie dodany kontakt. Każdy lead przechodzi przez cykl życia od pierwszego kontaktu aż do realizacji usługi lub utraty zainteresowania.\n\nWidok Leadów pozwala zarządzać tym procesem w jednym miejscu: śledzić status, przypisywać pracowników i klientów, tworzyć kosztorysy, rezerwacje oraz przygotowywać odpowiedzi e-mail.',
        usedIn: [],
      },
      {
        id: 'tiles',
        ...
        group: 'Wstęp',
        description: 'Cztery kafelki na górze strony pokazują kondycję procesu sprzedażowego w czasie rzeczywistym:\n\n• Do obsłużenia — liczba leadów czekających na pierwszy kontakt. Kliknięcie filtruje tabelę do statusu „Nowy".\n\n• Konwersja (ten miesiąc) — procent leadów zamienionych w klientów. Trend ze strzałką pokazuje zmianę (pp) względem poprzedniego miesiąca.\n\n• Zrealizowane (ten miesiąc) — suma wartości leadów zakończonych sukcesem. Kliknięcie filtruje do statusu „Zakończony".\n\n• Ryzyko utraty — suma wartości leadów bez aktywności od 24+ godzin. Kliknięcie filtruje do statusów „Nowy" i „W kontakcie", sortując po czasie ostatniej aktualizacji.',
      ...
      },
      {
        ...
        group: 'Przypisywanie',
        description: 'Lead może, ale nie musi, mieć przypisanego klienta z bazy CRM. Przypisanie łączy historię zapytania z kartoteką klienta (wizyty, przychód, pojazdy).\n\nPrzycisk „Przypisz klienta" (niebieski, z obwódką) otwiera wyszukiwarkę klientów. Jeśli klient nie istnieje jeszcze w systemie, dostępna jest opcja „Dodaj nowego klienta" — dane z leada (e-mail, telefon, imię i nazwisko) są wstępnie uzupełnione w formularzu.\n\nPo przypisaniu imię klienta jest klikalnym linkiem przenoszącym na stronę szczegółów klienta. Zmiana możliwa jest przez szary przycisk „Zmień przypisanie".',
      ...
      },
      {
        ...
        group: 'Przypisywanie',
        description: 'Każdy lead może mieć przypisanego pracownika odpowiedzialnego za obsługę kontaktu. Pozwala to śledzić kto prowadzi rozmowę i kto odpowiada za konwersję.\n\nPrzycisk „Przypisz pracownika" otwiera modal wyboru pracownika z listy. Przycisk „Przypisz siebie" skraca ten proces do jednego kliknięcia. Gdy pracownik jest już przypisany, wyświetla się jego imię z zielonym ptaszkiem i szary przycisk „Zmień przypisanie".\n\nFiltr „Pracownik" w pasku filtrów pozwala zawęzić widok do leadów przypisanych do konkretnej osoby.',
      ...
      },
      {
        ...
        group: 'Przypisywanie',
        description: 'Lead w statusie „Zarezerwowany" może mieć przypisaną rezerwację z kalendarza. Lead w statusie „Zakończony" może mieć przypisaną wizytę (zrealizowaną usługę).\n\nPrzycisk „Przypisz rez." lub „Przypisz wizytę" otwiera wyszukiwarkę odpowiednich wpisów. Po przypisaniu pojawia się zielony przycisk — kliknięcie w „Rezerwacja" przenosi do kalendarza, w „Wizyta" — do szczegółów wizyty.\n\nWizyta ma wyższy priorytet wyświetlania: jeśli lead ma przypisane obie, w tabeli pokazany jest przycisk „Wizyta".\n\nWażne: jedna rezerwacja lub wizyta może być przypisana tylko do jednego leada jednocześnie. Próba przypisania zajętej pozycji wyświetli ostrzeżenie z informacją, do którego leada jest już przypisana.',
      ...
      },
      {
        ...
        label: 'Kosztorys',
        group: 'Kosztorys',
        description: 'Dla leadów e-mail system automatycznie analizuje treść zapytania i tworzy kosztorys — listę usług z szacowanymi cenami dopasowanymi do marki i modelu pojazdu. To kosztorys systemowy, widoczny w szczegółach leada.\n\nMożesz też stworzyć własny kosztorys ręcznie — dodawać, usuwać i edytować pozycje, ceny i ilości. Jest on oznaczony jako „Twój kosztorys".\n\nPriorytet: „Twój kosztorys" ma pierwszeństwo przed kosztorysem systemowym we wszystkich obliczeniach wartości oraz przy pre-wypełnianiu formularza rezerwacji. Kosztorys systemowy pełni wtedy rolę podpowiedzi w tle.\n\nSzacowana wartość leada widoczna w kolumnie „Wartość" pochodzi z kosztorysu o wyższym priorytecie.',
      ...
      },
      {
        ...
        description: 'Dla leadów z zapytaniem e-mail dostępny jest przycisk „Przygotuj ofertę". Kliknięcie otwiera okno compose (w stylu macOS Mail) z przygotowaną treścią odpowiedzi.\n\nTreść jest personalizowana na podstawie danych leada: imienia klienta, marki i modelu pojazdu oraz wyceny z kosztorysu. Podczas generowania widoczna jest animacja pisania — tekst pojawia się stopniowo. Po zakończeniu można go swobodnie edytować.\n\nGotową wiadomość kopiujesz do schowka przyciskiem „Kopiuj do schowka" i wklejasz do swojego klienta pocztowego.\n\nJak działa integracja z e-mailem\n\nW ustawieniach firmy (pole „Alias e-mail") możesz ustawić swój unikalny alias — np. twojafirma@detailboost.pl. Aby zapytania e-mail trafiały do systemu, skonfiguruj w swojej skrzynce pocztowej automatyczne przekierowanie wiadomości na ten adres alias.\n\nAlias nie jest skrzynką pocztową — pełni rolę pośrednika. Gdy wiadomość dotrze na alias, serwer ją przetworzy (odczyta dane leada), a następnie całkowicie o niej zapomni. Serwer nie przechowuje treści e-maili i nie ma możliwości powrotu do oryginalnej wiadomości. Wysyłka odpowiedzi zawsze odbywa się po Twojej stronie, w Twoim kliencie pocztowym.',
        usedIn: ['Modal szczegółów leada — dla leadów e-mail', 'Ustawienia konta — pole Alias e-mail'],
      },
      {
        id: 'examples',
        label: 'Przykłady odpowiedzi',
        group: 'Tworzenie odpowiedzi na e-mail',
        description: 'Po przygotowaniu i edycji odpowiedzi możesz kliknąć „Zapisz jako przykład" — system zapisze tę wersję jako wzorzec stylu.\n\nPrzy kolejnych generowaniach system uwzględnia zapisane przykłady i dopasowuje do nich styl, ton i strukturę nowych odpowiedzi. Im więcej dobrych przykładów, tym trafniejsze kolejne propozycje.\n\nZarządzanie przykładami (podgląd, edycja, usuwanie) dostępne jest przez:\n• przycisk „Przykłady ofert" (ikona książki) w nagłówku strony,\n• ikonkę książki w oknie compose.\n\nLimit: maksymalnie 10 przykładów. Gdy limit jest osiągnięty, usuń jeden ze starych, aby dodać nowy.',
