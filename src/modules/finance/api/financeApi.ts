...
const mockDocuments: FinancialDocument[] = [
  {
    ...
    isCorrection: false,
    hasCorrection: false,
    correctionExternalId: null,
    providerSyncStatus: null,
    providerSyncStatusLabel: null,
...
export const financeApi = {
  ...
  // ── Invoice operations (provider integration) ────────────────────────────

  syncAllInvoices: async (): Promise<SyncInvoicesResult> => {
    const response = await apiClient.post(`${BASE_PATH}/invoices/sync`);
    return response.data;
  },

  syncSingleInvoice: async (id: string): Promise<FinancialDocument> => {
    const response = await apiClient.post(`${BASE_PATH}/invoices/${id}/sync`);
    return response.data;
  },

  importInvoices: async (): Promise<ImportInvoicesResult> => {
    const response = await apiClient.post(`${BASE_PATH}/invoices/import`);
    return response.data;
  },

  retrySyncInvoice: async (id: string): Promise<FinancialDocument> => {
    const response = await apiClient.post(`${BASE_PATH}/invoices/${id}/retry-sync`);
    return response.data;
  },

  getInvoicePortalUrl: async (id: string): Promise<string> => {
    const response = await apiClient.get(`${BASE_PATH}/invoices/${id}/portal-url`);
