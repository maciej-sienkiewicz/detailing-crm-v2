...
//
// Zmiana układu (wrzesień 2026): oś czasu przestała być przypisem pod wyceną i stała
// się główną treścią panelu szczegółów. Treść wiadomości stoi na niej wprost — nie ma
// przycisku podglądu, bo nie ma czego odsłaniać, a najnowsze zdarzenie jest na górze.
// Wcześniejsze testy opisywały stan odwrotny (treść schowana za ikoną oka, oś rosnąca)
// i zostały przepisane pod nową intencję, a nie dopasowane do kodu po fakcie.
 ...
 *
 * Kolejność jest ROSNĄCA, tak jak oddaje ją backend — komponent odwraca ją sam.
 */
...
describe('LeadTimeline', () => {
    ...
    it('treść każdej wiadomości stoi na osi wprost', () => {
    ...
    it('znacznik czasu mówi najpierw, jak dawno, a potem kiedy', () => {
        // „Czeka 6 dni" na karcie i wiersz osi czasu mają mówić o tym samym tymi
        // samymi słowami — inaczej trzeba przeliczać jedno na drugie w głowie.
        renderTimeline([
            entry({ id: '1', kind: 'OUTBOUND_MESSAGE', at: daysAgo(6), body: 'wycena w załączniku' }),
        ]);

        expect(screen.getByText(/^6 dni temu · /)).toBeTruthy();
    });

    it('kontakt poza pocztą pokazuje notatkę tak samo jak treść wiadomości', () => {
        renderTimeline([
            entry({
                id: '1',
                kind: 'CALLBACK',
