...
//
// Notatki o kliencie - ten sam panel szyny co notatki pojazdu i notatka techniczna
// wizyty: tytuł zwykłym pismem, „Dodaj" obrysowane (wypełnienie należy do „Nowa
// wizyta" w nagłówku), edycja w miejscu.
//
// Wcześniej autor i data były sklejone kropką („Marta · 10.05.2026"), przyciski
// edycji pojawiały się dopiero po najechaniu (na telefonie nigdy), a usunięcie
// pytało systemowym `confirm`.
...
export const CustomerNotes = ({ customerId, readOnly = false, id }: CustomerNotesProps) => {
    ...
    const [toDelete, setToDelete] = useState<CustomerNote | null>(null);
    ...
    return (
        <RailPanel id={id} aria-labelledby="customer-notes-title">
            <Head>
                <SectionTitle id="customer-notes-title" count={!isLoading && notes.length ? notes.length : undefined}>Notatki</SectionTitle>
                {!isAdding && !readOnly && (
            ...
            {isAdding && (
                <Form>
                    <Textarea
                        ...
                        placeholder="Np. woli kontakt SMS-em, dzwonić tylko po 16:00"
                    />
                    <FormActions>
            ...
            {isLoading ? (
                ...
                !isAdding && <Empty>Brak notatek o tym kliencie.</Empty>
            ) : (
                <List>
                    {notes.map(note => (
                        <Item key={note.id}>
            ...
            <ConfirmationModal
                ...
                message="Notatka zniknie z karty klienta. Tej operacji nie można cofnąć."
                variant="danger"
